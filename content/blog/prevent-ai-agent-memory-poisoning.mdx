...
![Diagram of untrusted inputs passing through an AI-agent memory write-admission gate](/blog/prevent-ai-agent-memory-poisoning-memory-write-admission-gate.png)
...
![Comparison of immediate prompt injection and delayed AI-agent memory poisoning](/blog/prevent-ai-agent-memory-poisoning-prompt-injection-versus-memory-poisoning.png)
...
![Flow diagram routing an AI-agent memory candidate to active memory, quarantine, or rejection](/blog/prevent-ai-agent-memory-poisoning-memory-quarantine-write-pipeline.png)
...
![Architecture diagram showing tenant, user, project, agent, and environment isolation around AI-agent memory](/blog/prevent-ai-agent-memory-poisoning-memory-isolation-boundaries.png)
...
![Diagram of security checks applied to AI-agent memory before it enters model context](/blog/prevent-ai-agent-memory-poisoning-memory-retrieval-security-gates.png)
...
![Lifecycle diagram showing AI-agent memory quarantine, expiry, deletion, restoration, and audit paths](/blog/prevent-ai-agent-memory-poisoning-memory-lifecycle-rollback.png)
...
![Security test loop for AI-agent memory poisoning, retrieval abuse, authorization, and recovery](/blog/prevent-ai-agent-memory-poisoning-memory-poisoning-abuse-test-loop.png)
